...
class TestCombat(unittest.TestCase):
    ...
    def test_reaper(self):
        ...
        combat = Combat(game_state, ['louse', 'blockman', 'louse'])
        combat.take_action('end_turn')
        reaper_id = get_key_by_substring(combat.player['hand'], 'reaper')
        combat.take_action(f"play {reaper_id}")
        self.assertEqual(game_state['player']['hp'], 98)
        self.assertEqual(combat.player['energy'], 8)
        self.assertEqual(len(combat.player['hand']), 4)
        self.assertEqual(len(combat.player['exhaust_pile']), 1)
